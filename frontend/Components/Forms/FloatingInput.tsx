    ...
    <>
        ...
        >
            <input
                ...
                value={value}
                required={required}
                className="text-black [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none p-2 peer w-full border-none bg-transparent placeholder-transparent focus:border-transparent focus:outline-none focus:ring-0"
                placeholder={label}
            />
                <span
        ...
        </label>
        {
            errors?.map((error, idx)=>
                <span key={idx} className='text-red-500 block'>{error}</span>
            )
        }
    </>
  )
